...
import '../screens/payments/widgets/widgets.dart';



/// Allows to externally control [PaginatedTable] state
...
/// pass it to [PaginatedTable.controller] via constructor and you're ready to go.
...
class PaginatorController extends ChangeNotifier {
  ...
  void _attach(PaginatedTableState state) {
    _state = state;
  }
  ...
  /// Returns number of rows displayed in the [PaginatedTable]. Throws if no
  ...
  /// Returns number of rows displayed in single page of the [PaginatedTable].
  ...
  /// Returns the index of the first (topmost) row displayed currently displayed in [PaginatedTable].
  /// Throws if no table is attached to the controller
...
class PaginatedTable extends StatefulWidget {
  ...
  /// Note: to change background color of fixed data rows use [PaginatedTable.headingRowColor] and
  ...
  /// your own paginator and control the widget via [PaginatedTable.controller]
      ...
      Row(
        children: [
          ActionIconElement(
              onTap: () {
                setState(() {
                  widget.dataRowHeight = 30;
                });
              },
              isAvailable: widget.dataRowHeight != 30,
              icon: const Icon(Icons.zoom_out),
              actionName: 'Уменьшить размер колонок'),
      ActionIconElement(
          onTap: () {
            setState(() {
              widget.dataRowHeight = widget.dataRowHeight * 2;
            });
          },
          isAvailable: true,
          icon: const Icon(Icons.zoom_in),
          actionName: 'Увеличить размер колонок'),                                   
        ],
      ), 
      if(widget.additionalElements != null)
      widget.additionalElements ?? const SizedBox.shrink()         
    ]);
