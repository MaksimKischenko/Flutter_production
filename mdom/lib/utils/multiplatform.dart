...
void _log(dynamic message) =>
    Logger.helpPayLog(message, name: '_multiplatform');
...
class Multiplatform {
  ...
  static Future<DateTime?> showOnlyTimePicker({
    required BuildContext context,
    ...
    if (isIOS) {
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: CupertinoDatePicker(
                      ...
                      minimumDate:
                          initialTime.subtract(const Duration(days: 1)),
      ...
      if (time == null) {
        ...
        return DateTime(initialTime.year, initialTime.month, initialTime.day,
            time.hour, time.minute, 59);
      }
    }
  }
  ...
  static Future<T?> showDropdown<T>({
    ...
    required String Function(T value) builder,
    T? selected,
    String? title,
  }) {
    if (isIOS) {
      final selectedIndex = selected == null ? 0 : items.indexOf(selected);
      return showModalBottomSheet<T>(
        backgroundColor: const Color(0xffd1d1d1), //.withOpacity(0.9),
        context: context,
        builder: (context) {
          T? selectedItem = selected ?? items[0];
          return Theme(
            data: Theme.of(context).copyWith(textTheme: iosTextTheme),
            child: SafeArea(
              child: Column(
  ...
  static void showPinScreen({
    ...
    screenLock(
      ...
      delayBuilder: (context, delay) =>
          Text('Невозможно войти в течение ${delay.inSeconds} секунд'),
      customizedButtonChild: canBiometric
          ? komplatBiometricType == KomplatBiometricType.faceid
              ...
              : const Icon(Icons.fingerprint,
                  size: 36, color: AppStyles.mainTextColor)
          : null,
  ...
  static void showVerifyPinScreen({
    ...
    screenLockCreate(
      context: context,
      title: Text(title, style: const TextStyle(color: AppStyles.mainColorDark, fontSize: 16)),
      confirmTitle: const Text('Повторите новый PIN', style: TextStyle(color: AppStyles.mainColorDark, fontSize: 16)),
      config: ScreenLockConfig(
          backgroundColor: Colors.white,
          themeData: ThemeData(
            fontFamily: 'Roboto',
    ...
    required Widget Function(BuildContext context) builder,
      ...
      showDialog<T>(
        ...
        builder: (context) {
          ...
          return AnimatedPadding(
            padding: effectivePadding, //padding ?? const EdgeInsets.all(100),
            duration: const Duration(milliseconds: 100),
            curve: Curves.decelerate,
            child: ClipRRect(
