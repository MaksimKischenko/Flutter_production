  ...
  Widget build(BuildContext context) => BlocBuilder<MdomAccountInfoBloc, MdomAccountInfoState>(
    builder: (context, state) {
      ...
      if (state is MdomAccountInfoLoading) {
        ...
        final balance = state.periodInfo?.servicesWrapper?.services?.fold<Decimal>(Decimal.zero, (sum, service) => sum + Decimal.parse((service.balance ?? 0.0).toString())) ?? Decimal.zero;
        final fine = state.periodInfo?.servicesWrapper?.services?.fold<Decimal>(Decimal.zero, (sum, service) => sum + Decimal.parse((service.fine ?? 0.0).toString())) ?? Decimal.zero;
        ...
        final payAmount = (balance + fine + totalSum - paymentSum).toDouble();
        ...
        if(payAmount >= 0) {
          ...
          body  = _info(
            text: context.t.mobileScreens.mdomAccountInfo.payButton.noNeedPay(
